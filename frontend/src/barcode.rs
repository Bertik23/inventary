...

    {
        ...
        use_effect_with((), move |_| {
            ...
            spawn_local(async move {
                if let Some(video) = video_ref_async.cast::<HtmlVideoElement>()
                {
                        ...
                        .get_user_media_with_constraints(&constraints)
                    {
                        Ok(stream_promise) => {
                            ...
                            .await
                            {
                                Ok(stream_value) => {
                                    ...
                                    start_barcode_detection(
                                        video, canvas_ref, on_scan, scanning,
                                        error,
                                    )
                                    .await;
                                }
                        ...
                        Err(e) => {
                            let error_msg =
                                format!("Failed to get user media: {:?}", e);
                            log::error!("{}", error_msg);
                            error.set(Some(error_msg));
                            scanning.set(false);
                        }
            ...
            move || {
                ...
                if let Some(video) =
                    video_ref_cleanup.cast::<HtmlVideoElement>()
                {
                    if let Some(stream) = video
                        .src_object()
                        .and_then(|s| s.dyn_ref::<MediaStream>().cloned())
                    {
                        for track_js in stream.get_tracks().iter() {
                            if let Some(track) =
                                track_js.dyn_ref::<MediaStreamTrack>()
                            {
                                track.stop();
                            }
                        }
...
async fn start_barcode_detection(
    ...
    let barcode_detector_available =
        Reflect::has(&window, &JsValue::from_str("BarcodeDetector"))
            .unwrap_or(false);

    if barcode_detector_available {
        // Use native BarcodeDetector API
        detect_with_barcode_detector(
            video, canvas_ref, on_scan, scanning, error,
        )
        .await;
    } else {
...
async fn detect_with_barcode_detector(
    ...
    let barcode_detector_ctor =
        Reflect::get(&window, &JsValue::from_str("BarcodeDetector")).ok();
    ...
    let detector_ctor_fn: Function = match barcode_detector_ctor
        .unwrap()
        .dyn_into()
    {
        Ok(f) => f,
    ...
    let detector = Reflect::construct(
        &detector_ctor_fn,
        &js_sys::Array::from_iter([JsValue::from(&formats)]),
    )
    .ok();
    ...
    loop {
        ...
        let detect_fn =
            Reflect::get(&detector, &JsValue::from_str("detect")).ok();
        if let Some(detect_method_js) = detect_fn {
            if let Ok(detect_method) = detect_method_js.dyn_into::<Function>() {
                let detect_result = Reflect::apply(
                    &detect_method,
                    &detector,
                    &js_sys::Array::from_iter([JsValue::from(&canvas)]),
                );

                if let Ok(result_promise) = detect_result {
                    let result: js_sys::Promise =
                        result_promise.dyn_into().unwrap();
                    match wasm_bindgen_futures::JsFuture::from(result).await {
                        Ok(barcodes) => {
                            let barcodes_array: js_sys::Array =
                                barcodes.dyn_into().unwrap();
                            if barcodes_array.length() > 0 {
                                let first_barcode = barcodes_array.get(0);
                                if let Ok(raw_value) = Reflect::get(
                                    &first_barcode,
                                    &JsValue::from_str("rawValue"),
                                ) {
                                    if let Some(barcode_str) =
                                        raw_value.as_string()
                                    {
                                        log::info!(
                                            "Barcode detected: {}",
...
async fn detect_with_zxing(
    ...
    let window = web_sys::window().unwrap();
    let zxing_available =
        Reflect::has(&window, &JsValue::from_str("ZXing")).unwrap_or(false);
    ...
    let code_reader_ctor =
        Reflect::get(&zxing, &JsValue::from_str("BrowserMultiFormatReader"))
            .ok();

    let reader = if let Some(ctor) = code_reader_ctor {
        if let Ok(reader_instance) = Reflect::construct(
            &ctor.dyn_into::<Function>().unwrap(),
            &js_sys::Array::new(),
        ) {
            reader_instance
        } else {
        ...
        error.set(Some(
            "ZXing BrowserMultiFormatReader not available".to_string(),
        ));
        scanning.set(false);
        return;
    };
    ...
    loop {
        ...
        let decode_fn_js =
            Reflect::get(&reader, &JsValue::from_str("decodeFromCanvas")).ok();
        ...
        if let Some(decode_method_js) = decode_fn_js {
            if let Ok(decode_method) = decode_method_js.dyn_into::<Function>() {
                ...
                if let Ok(result) = Reflect::apply(
                    &decode_method,
                    &reader,
                    &js_sys::Array::from_iter([JsValue::from(&canvas)]),
                ) {
                    let result_promise: js_sys::Promise =
                        result.dyn_into().unwrap();
                    match wasm_bindgen_futures::JsFuture::from(result_promise)
                        .await
                    {
                        Ok(decode_result) => {
                            if let Ok(text) = Reflect::get(
                                &decode_result,
                                &JsValue::from_str("text"),
                            ) {
                                if let Some(barcode_str) = text.as_string() {
                                    log::info!(
                                        "Barcode detected with ZXing: {}",
                                        barcode_str
                                    );
                                    on_scan.emit(barcode_str);
                                    scanning.set(false);
